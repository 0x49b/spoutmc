...
func getMojangData(displayname string) (uuid.UUID, string, error) {
	// Basic validation for Minecraft username (alphanumeric + underscore, 3-16 chars)
	matched, _ := regexp.MatchString(`^[a-zA-Z0-9_]{3,16}$`, displayname)
	if !matched {
		return uuid.Nil, "", fmt.Errorf("invalid display name format")
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		return uuid.Nil, "", fmt.Errorf("API returned status: %s", resp.Status)
	}
	...
	if !data.Success || data.Data.Player.RawID == "" {
